...
function trainTestSplit(features, targets, testSize = 0.2) {
...
function preprocessData() {
    try {
        ...
        // Shuffle and split data BEFORE preprocessing (FIXED DATA LEAKAGE)
        const split = trainTestSplit(features, targets, 0.2);
        
        // Process training data
        const processedTrainFeatures = imputeMissingValues(split.trainFeatures);
        ...
        // Standardize using only training data statistics (FIXED DATA LEAKAGE)
        ...
        appState.standardizationParams = {
            means: means,
            stds: stds,
            featureNames: [...SCHEMA.quantitative, ...Object.keys(SCHEMA.derivedFeatures)]
        };
        
        // Process validation data using training statistics (FIXED DATA LEAKAGE)
        const processedValFeatures = imputeMissingValues(split.testFeatures);
        ...
        const quantitativeValData = engineeredValFeatures.map(row => 
            appState.standardizationParams.featureNames.map(feature => row[feature])
        );
        const standardizedValData = applyStandardization(quantitativeValData, means, stds);
        
        appState.processedData = {
            trainFeatures: standardizedTrainData,
            trainTargets: split.trainTargets,
            valFeatures: standardizedValData,
            valTargets: split.testTargets,
            featureNames: appState.standardizationParams.featureNames,
            trainIndices: split.trainIndices,
            valIndices: split.testIndices
        };
        
        showPreprocessingDetails();
        showStatus(elements.preprocessStatus(), 
            `✅ Advanced preprocessing complete! ${standardizedTrainData[0].length} engineered features (No Data Leakage)`, 'success');
        ...
        elements.featureInfo().innerHTML = `
            ...
            <div style="margin-top: 8px;">
                ...
                • Training samples: ${standardizedTrainData.length}<br>
                • Validation samples: ${standardizedValData.length}<br>
                • Data properly shuffled and split
            </div>
        `;
...
function applyStandardization(data, means, stds) {
    return data.map(row => {
        return row.map((value, index) => {
            return isNaN(value) ? 0 : (value - means[index]) / stds[index];
        });
    });
}

function showPreprocessingDetails() {
    if (!appState.standardizationParams) return;
    
    let details = `<strong>📊 Preprocessing Details (No Data Leakage):</strong><br>`;
    details += `• Features engineered: ${Object.keys(SCHEMA.derivedFeatures).length}<br>`;
    details += `• Standardization applied using training data only<br>`;
    details += `• Data properly shuffled before splitting<br>`;
    
    if (Object.keys(appState.standardizationParams.means).length > 0) {
        details += `<br><strong>Standardization Parameters (from training data):</strong><br>`;
        appState.standardizationParams.featureNames.slice(0, 3).forEach((feature, index) => {
            details += `• ${feature}: mean=${appState.standardizationParams.means[index].toFixed(2)}, std=${appState.standardizationParams.stds[index].toFixed(2)}<br>`;
        });
        if (appState.standardizationParams.featureNames.length > 3) {
            details += `• ... and ${appState.standardizationParams.featureNames.length - 3} more features`;
        }
    }
...
async function trainModel() {
    try {
        ...
        const features = tf.tensor2d(appState.processedData.trainFeatures);
        const targets = tf.tensor1d(appState.processedData.trainTargets);
        const valFeatures = tf.tensor2d(appState.processedData.valFeatures);
        const valTargets = tf.tensor1d(appState.processedData.valTargets);
        ...
        const history = await appState.model.fit(features, targets, {
            epochs: epochs,
            batchSize: batchSize,
            validationData: [valFeatures, valTargets], // Use explicit validation data (FIXED DATA LEAKAGE)
            verbose: 0,
            callbacks: {
                    ...
                    if (epoch === epochs - 1) {
                        ...
                        elements.trainingProgress().innerHTML = `
                            <div class="training-results">
                                <strong>Training Complete! (No Data Leakage)</strong><br>
                                Final Training Loss: ${logs.loss.toFixed(4)}<br>
                                Final Training Accuracy: ${(logs.acc * 100).toFixed(1)}%<br>
        ...
        const predictions = appState.model.predict(valFeatures);
        const predictedValues = await predictions.data();
        
        appState.validationPredictions = Array.from(predictedValues);
        appState.validationLabels = Array.from(await valTargets.data());
        
        calculateAndDisplayMetrics();
        calculateFeatureImportance();
        
        showStatus(elements.trainingStatus(), 
            `✅ Training complete! (Clean validation metrics)`, 
            'success');
        
        updateUIState();
        
        tf.dispose([features, targets, valFeatures, valTargets, predictions]);
...
function calculateFeatureImportance() {
    ...
    const features = appState.processedData.trainFeatures;
    const targets = appState.processedData.trainTargets;
    ...
    for (let i = 0; i < numFeatures; i++) {
        ...
        for (let j = 0; j < featureValues.length; j++) {
            numerator += (featureValues[j] - featureMean) * (targets[j] - targetMean);
            denomX += Math.pow(featureValues[j] - featureMean, 2);
            denomY += Math.pow(targets[j] - targetMean, 2);
        }
...
function displayFeatureImportance() {
    ...
    let html = `<strong>🎯 Feature Correlation Analysis:</strong><br><br>`;
...
async function predictTestData() {
    try {
        ...
        const standardizedTestData = applyStandardization(
            quantitativeTestData, 
            appState.standardizationParams.means, 
            appState.standardizationParams.stds
        );
...
// ==================== IMPROVED MODEL EXPORT FUNCTION ====================
...
async function exportModel() {
    ...
    try {
        showStatus(elements.modelStatus(), 'Exporting model and preprocessing parameters...', 'loading');
        
        // Prepare model data for export
        const modelData = {
            modelTopology: appState.model.modelTopology,
            weightSpecs: appState.model.weights.map(w => w.spec),
            weightData: await Promise.all(appState.model.weights.map(w => w.data())),
            trainingConfig: appState.model.trainingConfig,
            preprocessing: {
                standardizationParams: appState.standardizationParams,
                schema: SCHEMA,
                featureNames: appState.processedData.featureNames
            },
            metadata: {
                exportDate: new Date().toISOString(),
                trainingMetrics: appState.finalTrainingMetrics,
                featureImportance: appState.featureImportance
            }
        };
        
        // Convert weight data to arrays
        const weightArrays = await Promise.all(modelData.weightData);
        modelData.weightData = weightArrays.map(arr => Array.from(arr));
        
        const blob = new Blob([JSON.stringify(modelData, null, 2)], { type: 'application/json' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `loan_approval_model_${new Date().toISOString().split('T')[0]}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);
        
        showStatus(elements.modelStatus(), '✅ Model exported successfully! Includes weights and preprocessing parameters.', 'success');
        
    } catch (error) {
        showStatus(elements.modelStatus(), `❌ Model export error: ${error.message}`, 'error');
...
async function importModel(event) {
    ...
    try {
        ...
        appState.model = await tf.loadLayersModel(tf.io.fromMemory(
            modelData.modelTopology,
            modelData.weightSpecs,
            modelData.weightData.map(data => Float32Array.from(data))
        ));
        
        // Restore preprocessing parameters
        appState.standardizationParams = modelData.preprocessing.standardizationParams;
        appState.processedData = {
            featureNames: modelData.preprocessing.featureNames
        };
        appState.finalTrainingMetrics = modelData.metadata.trainingMetrics;
        appState.featureImportance = modelData.metadata.featureImportance;
        
        // Update UI
        elements.modelSummary().innerHTML = `
            <strong>🧠 Loaded Model Architecture:</strong>
            <div style="background: #f8f9fa; padding: 10px; border-radius: 5px; font-size: 0.9rem; margin-top: 10px;">
                <strong>Imported Model:</strong><br>
                • Input: ${modelData.preprocessing.featureNames.length} features<br>
                • Layers: ${modelData.modelTopology.config.layers.length} layers<br>
                • Export Date: ${new Date(modelData.metadata.exportDate).toLocaleDateString()}<br>
                • Training Accuracy: ${modelData.metadata.trainingMetrics?.accuracy || 'N/A'}%<br>
                • Validation Accuracy: ${modelData.metadata.trainingMetrics?.val_accuracy || 'N/A'}%
            </div>
        `;
        ...
        showStatus(elements.modelStatus(), '✅ Model loaded successfully! Ready for predictions.', 'success');
        
    } catch (error) {
        showStatus(elements.modelStatus(), `❌ Model import error: ${error.message}`, 'error');
