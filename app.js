...
function showChart(chartType) {
    ...
    if (appState.charts[chartType]) {
        // Recreate the chart on the new canvas
        const ctx = canvas.getContext('2d');
        const chartConfig = JSON.parse(JSON.stringify(appState.charts[chartType].config));
        appState.charts[chartType].destroy();
        appState.charts[chartType] = new Chart(ctx, chartConfig);
    }
}
